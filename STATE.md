...
**Generated 2026-09-13 03:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.97s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260913-011247-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 374h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 408h old (bar 30h) |
| delivery | RED | 792 commits on no remote (oldest 20.5d), 21 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 391 commits no remote holds, oldest 9.7d, 13 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 20.5d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 19.3d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 9915c23, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2701 producers, 12195 measurables, 379 in gaps with a ticket, 23 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 465h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 1503 runs in 24h, 142 refused (most: pre-commit 109), slowest 0 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 345.8h ago (bar 194h)) |
...
| OKE flux | RED | 7 of 85 Kustomizations not ready: chaos, commerce, cyrus, guacamole, tailscale, temporal, To |
