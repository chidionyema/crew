...
**Generated 2026-08-30 19:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 1.94s |
...
| maestro | GREEN | last cycle 6 min ago (`INTENT-20260830-185056-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 54h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 88h old (bar 30h) |
| delivery | RED | 1524 commits on no remote (oldest 8.4d), 48 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 828 commits no remote holds, oldest 6.0d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-vocab | 351 commits no remote holds, oldest 7.2d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 8.4d, 4 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main c852db4, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 5224 producers, 46067 measurables, 270 in gaps with a ticket, 16 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 145h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 48285 runs in 24h, 357 refused (most: rule-guard.py 125), slowest 35866 ms, 16 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 25.8h ago (bar 194h)) |
