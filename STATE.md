...
**Generated 2026-09-08 11:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 2.20s |
...
| estate board | GREEN | estate-board-sync: 214 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 110 min ago (`INTENT-20260908-091338-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 262h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 296h old (bar 30h) |
| delivery | RED | 633 commits on no remote (oldest 15.8d), 45 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 357 commits no remote holds, oldest 15.8d, 0 dirty |
| &nbsp;&nbsp;stranded | idp | 240 commits no remote holds, oldest 5.0d, 11 dirty |
...
| live checkout | GREEN | moved 1 commit(s) to origin/main fa391b9, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1958 producers, 8563 measurables, 374 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 353h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 38475 runs in 24h, 285 refused (most: rule-guard.py 123), slowest 25508 ms, 14 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 233.8h ago (bar 194h)) |
...
| OKE flux | RED | 55 of 81 Kustomizations not ready: agent-workforce, alerts, alerts-github, alerts-secret, autoscaler, backstage, chaos, cluster-state, commerce, commerce-data, crossplane-providers, cyrus, dagster, dns, drills, estate-db, estate-db-migrate, external-secrets, flux-webhook, guacamole, gvisor-runtime, healing-analyzer, healing-k8sgpt, healthchecks, hermes-agent, hindsight, human-vault, human-vault-bridge, identity, image-automation, jit, llm, mcp, monitoring, monitoring-rules, nodesoftware-operator, notify, observability, otto-gateway, otto-golden, otto-golden-secret, prospector, prospector-platform, rbac, rbac-identity, reloader, research-engine, robusta, science, secret-store, tailscale, temporal, verification, weave-gitops, To |
