...
**Generated 2026-08-30 13:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | GREEN | last cycle 2 min ago (`INTENT-20260830-125521-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 48h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 82h old (bar 30h) |
| delivery | RED | 1522 commits on no remote (oldest 8.1d), 48 dirty files, 7 live repos (`git log --branches --not --remotes`) |
...
| &nbsp;&nbsp;stranded | .wt-vocab | 351 commits no remote holds, oldest 6.9d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 8.1d, 4 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 2 commit(s) to origin/main 9d2643b, 4 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 139h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 70535 runs in 24h, 617 refused (most: rule-guard.py 174), slowest 40422 ms, 22 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 19.8h ago (bar 194h)) |
