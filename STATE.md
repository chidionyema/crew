...
**Generated 2026-09-12 23:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.02s |
...
| maestro | RED | last cycle 1071 min ago (`INTENT-20260912-051212-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 370h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 404h old (bar 30h) |
| delivery | RED | 785 commits on no remote (oldest 20.3d), 15 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 384 commits no remote holds, oldest 9.5d, 8 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 20.3d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 19.1d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 5134bed, 3 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 461h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 1117 runs in 24h, 122 refused (most: pre-commit 96), slowest 0 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 341.8h ago (bar 194h)) |
