...
**Generated 2026-09-11 19:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 1.86s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260911-171228-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 342h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 376h old (bar 30h) |
| delivery | RED | 784 commits on no remote (oldest 19.2d), 11 dirty files, 10 live repos (`git log --branches --not --remotes`) |
...
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 18.0d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main c17fe26, 4 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 433h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 4466 runs in 24h, 15 refused (most: rule-guard.py 6), slowest 22020 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 313.8h ago (bar 194h)) |
