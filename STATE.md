...
**Generated 2026-09-11 11:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.41s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260911-091249-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 334h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 368h old (bar 30h) |
| delivery | RED | 778 commits on no remote (oldest 18.8d), 12 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 378 commits no remote holds, oldest 8.0d, 4 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 18.8d, 0 dirty |
...
| live checkout | GREEN | moved 1 commit(s) to origin/main 28d44be, 4 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 425h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 12251 runs in 24h, 30 refused (most: rule-guard.py 14), slowest 22020 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 305.8h ago (bar 194h)) |
...
| OKE flux | RED | 6 of 84 Kustomizations not ready: commerce, cyrus, guacamole, tailscale, temporal, To |
