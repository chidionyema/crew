...
**Generated 2026-09-10 09:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.57s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260910-071237-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 308h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 342h old (bar 30h) |
| delivery | RED | 774 commits on no remote (oldest 17.7d), 44 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 373 commits no remote holds, oldest 6.9d, 37 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 17.7d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 16.6d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 554f224, 3 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 399h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 10335 runs in 24h, 43 refused (most: rule-guard.py 25), slowest 43007 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 279.8h ago (bar 194h)) |
...
| OKE flux | RED | 18 of 82 Kustomizations not ready: agent-workforce, backstage, chaos, commerce, cyrus, dagster, guacamole, healing-analyzer, healing-k8sgpt, healthchecks, hindsight, observability, otto-gateway, research-engine, science, tailscale, temporal, To |
