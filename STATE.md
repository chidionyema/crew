...
**Generated 2026-08-31 05:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.25s |
...
| maestro | GREEN | last cycle 0 min ago (`INTENT-20260831-045717-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 64h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 98h old (bar 30h) |
| delivery | RED | 1564 commits on no remote (oldest 8.8d), 48 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 838 commits no remote holds, oldest 6.4d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-vocab | 351 commits no remote holds, oldest 7.6d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 8.8d, 4 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 568537d, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 4968 producers, 43333 measurables, 282 in gaps with a ticket, 16 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 155h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 53034 runs in 24h, 302 refused (most: rule-guard.py 115), slowest 28177 ms, 35 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 35.8h ago (bar 194h)) |
