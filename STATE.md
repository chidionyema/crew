...
**Generated 2026-09-11 01:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.34s |
...
| estate board | GREEN | estate-board-sync: 248 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | RED | last cycle 351 min ago (`INTENT-20260910-191218-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 324h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 358h old (bar 30h) |
| delivery | RED | 776 commits on no remote (oldest 18.4d), 54 dirty files, 11 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 376 commits no remote holds, oldest 7.6d, 38 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 18.4d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 17.2d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main f3bbd3f, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2334 producers, 10346 measurables, 370 in gaps with a ticket, 35 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 415h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 16574 runs in 24h, 39 refused (most: rule-guard.py 21), slowest 22020 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 295.8h ago (bar 194h)) |
...
| elite grade | 20 GAP, 15 BLIND | 378 ELITE of 413 entities; page docs/SHOWCASE.md in idp, gaps first (crew#474) |
