...
**Generated 2026-08-28 19:04 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 2.44s |
...
| maestro | GREEN | last cycle 6 min ago (`INTENT-20260828-185317-0d20f3e7.json`) |
...
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 40h old (bar 30h) |
| delivery | RED | 1445 commits on no remote (oldest 6.4d), 54 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 676 commits no remote holds, oldest 4.0d, 1 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 371 commits no remote holds, oldest 6.4d, 5 dirty |
| &nbsp;&nbsp;stranded | scripts | 349 commits no remote holds, oldest 5.2d, 21 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main d4a6e0c, 5 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 97h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 128379 runs in 24h, 676 refused (most: idle-guard.py 168), slowest 104612 ms, 24 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 2/38 layers on a cluster with no OCI (floor 2) (idp run 33201154933, 0.3h ago (bar 194h)) |
| &nbsp;&nbsp;second provider | | provider=github-hosted-azure distro=k3s wall_clock=674s cost=£0.00 (public repository, GitHub-hosted ubuntu-latest) |
