...
**Generated 2026-09-10 15:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.69s |
...
| maestro | STALE | last cycle 110 min ago (`INTENT-20260910-131333-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 314h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 348h old (bar 30h) |
| delivery | RED | 776 commits on no remote (oldest 18.0d), 20 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | NOT RUN | `gh search prs --merged` did not answer |
| &nbsp;&nbsp;stranded | idp | 376 commits no remote holds, oldest 7.2d, 12 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 18.0d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 16.8d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 58a8792, 4 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 405h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 16022 runs in 24h, 49 refused (most: rule-guard.py 26), slowest 22596 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 285.8h ago (bar 194h)) |
...
| OKE flux | RED | 14 of 83 Kustomizations not ready: agent-workforce, commerce, cyrus, guacamole, healing-analyzer, healing-k8sgpt, hermes-agent, hindsight, llm, research-engine, spire, tailscale, temporal, To |
