...
**Generated 2026-09-08 21:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 2.62s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260908-191211-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 272h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 306h old (bar 30h) |
| delivery | RED | 747 commits on no remote (oldest 16.2d), 9 dirty files, 10 live repos (`git log --branches --not --remotes`) |
...
| &nbsp;&nbsp;stranded | idp | 353 commits no remote holds, oldest 5.4d, 4 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 15.1d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 88c7db3, 2 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1848 producers, 8013 measurables, 374 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 363h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 33877 runs in 24h, 273 refused (most: pre-commit 122), slowest 25508 ms, 28 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 243.8h ago (bar 194h)) |
...
| OKE flux | RED | 4 of 82 Kustomizations not ready: commerce, prospector, temporal, To |
