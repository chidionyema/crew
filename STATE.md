...
**Generated 2026-09-13 01:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.90s |
...
| estate board | GREEN | estate-board-sync: 270 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 111 min ago (`INTENT-20260912-231307-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 372h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 406h old (bar 30h) |
| delivery | RED | 786 commits on no remote (oldest 20.4d), 15 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 385 commits no remote holds, oldest 9.6d, 7 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 20.4d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 19.2d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 6c4824b, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2651 producers, 11945 measurables, 379 in gaps with a ticket, 23 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 463h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 1326 runs in 24h, 137 refused (most: pre-commit 105), slowest 0 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 343.8h ago (bar 194h)) |
...
| laptop VM (R26) | RED | 50396 /usr/local/bin/limactl; stop it, do not restart it |
| OKE nodes | RED | `kubectl get nodes` -> 2/3 Ready (v1.35.2) |
| OKE flux | RED | 10 of 85 Kustomizations not ready: backstage, chaos, commerce, cyrus, edge, guacamole, jit, tailscale, temporal, To |
| elite grade | 20 GAP, 16 BLIND | 384 ELITE of 420 entities; page docs/SHOWCASE.md in idp, gaps first (crew#474) |
