...
**Generated 2026-09-07 21:00 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | RED | last cycle 4286 min ago (`INTENT-20260904-212911-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 248h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 282h old (bar 30h) |
| delivery | RED | 648 commits on no remote (oldest 15.2d), 49 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 352 commits no remote holds, oldest 15.2d, 0 dirty |
| &nbsp;&nbsp;stranded | idp | 260 commits no remote holds, oldest 4.4d, 14 dirty |
...
| live checkout | GREEN | moved 3 commit(s) to origin/main ab835b73, 5 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2001 producers, 8796 measurables, 364 in gaps with a ticket, 18 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 339h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 10289 runs in 24h, 54 refused (most: rule-guard.py 38), slowest 20435 ms, 4 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 219.7h ago (bar 194h)) |
...
| OKE flux | RED | 8 of 78 Kustomizations not ready: chaos, commerce, gvisor-runtime, healthchecks, mcp, nodesoftware-operator, temporal, To |
