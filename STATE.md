...
**Generated 2026-09-09 07:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | RED | last cycle 231 min ago (`INTENT-20260909-031237-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 282h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 316h old (bar 30h) |
| delivery | RED | 772 commits on no remote (oldest 16.7d), 26 dirty files, 9 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 372 commits no remote holds, oldest 5.8d, 19 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 16.7d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 15.5d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 1ab41ca, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1943 producers, 8486 measurables, 376 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 373h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 26037 runs in 24h, 281 refused (most: pre-commit 123), slowest 25530 ms, 42 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 253.8h ago (bar 194h)) |
...
| OKE flux | RED | 24 of 82 Kustomizations not ready: agent-workforce, alerts, backstage, chaos, commerce, cyrus, guacamole, gvisor-runtime, healing-analyzer, healing-k8sgpt, healthchecks, hindsight, llm, mcp, monitoring-rules, nodesoftware-operator, notify, observability, otto-gateway, otto-golden, research-engine, science, temporal, To |
