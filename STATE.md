...
**Generated 2026-09-11 21:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 1.96s |
...
| estate board | GREEN | estate-board-sync: 260 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 111 min ago (`INTENT-20260911-191214-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 344h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 378h old (bar 30h) |
| delivery | RED | 784 commits on no remote (oldest 19.2d), 10 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 383 commits no remote holds, oldest 8.4d, 3 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 19.2d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 12 commits no remote holds, oldest 18.1d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 778d75e, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2348 producers, 10429 measurables, 375 in gaps with a ticket, 23 unexplained, blind: cluster_live (`science/datamap.py --check`) |
| &nbsp;&nbsp;violation | | source ledger: owner ~/.claude/scripts/goal-guard.py does not exist |
| &nbsp;&nbsp;violation | | 23 producer(s) UNEXPLAINED (first: mac/ledger/~/.claude/state/command-timings.jsonl) |
| &nbsp;&nbsp;violation | | domain cluster_live BLIND and not allowed: RuntimeError: no receipt body in the last 8 oke-check.yml run(s): j |
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 435h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 3156 runs in 24h, 12 refused (most: pre-commit 5), slowest 22020 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 315.8h ago (bar 194h)) |
...
| OKE flux | RED | 25 of 84 Kustomizations not ready: agent-workforce, backstage, chaos, commerce, cyrus, dagster, drills, estate-db-migrate, guacamole, healing-analyzer, healing-k8sgpt, healthchecks, hermes-agent, hindsight, human-vault-bridge, llm, mcp, observability, otto-gateway, otto-golden, research-engine, science, tailscale, temporal, To |
