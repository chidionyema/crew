...
**Generated 2026-09-01 05:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 10.90s |
...
| maestro | GREEN | last cycle 5 min ago (`INTENT-20260901-045350-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 88h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 122h old (bar 30h) |
| delivery | RED | 1597 commits on no remote (oldest 9.8d), 25 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 849 commits no remote holds, oldest 7.4d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 351 commits no remote holds, oldest 8.6d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 9.8d, 5 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 5 commit(s) to origin/main 9c99903, 5 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 4157 producers, 35090 measurables, 277 in gaps with a ticket, 17 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 179h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 50071 runs in 24h, 470 refused (most: rule-guard.py 126), slowest 120389 ms, 60 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 59.8h ago (bar 194h)) |
