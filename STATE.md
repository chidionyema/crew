...
**Generated 2026-09-08 19:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| estate board | GREEN | estate-board-sync: 216 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 111 min ago (`INTENT-20260908-171215-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 270h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 304h old (bar 30h) |
| delivery | RED | 634 commits on no remote (oldest 16.2d), 76 dirty files, 14 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 357 commits no remote holds, oldest 16.2d, 0 dirty |
| &nbsp;&nbsp;stranded | idp | 241 commits no remote holds, oldest 5.3d, 32 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 15.0d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 0482756, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1960 producers, 8573 measurables, 374 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 361h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 40993 runs in 24h, 307 refused (most: rule-guard.py 136), slowest 25508 ms, 22 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 241.8h ago (bar 194h)) |
...
| OKE flux | RED | 7 of 82 Kustomizations not ready: commerce, gvisor-runtime, nodesoftware-operator, otto-gateway, prospector, temporal, To |
| elite grade | 24 GAP, 14 BLIND | 385 ELITE of 423 entities; page docs/SHOWCASE.md in idp, gaps first (crew#474) |
