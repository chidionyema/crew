...
**Generated 2026-09-07 23:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.27s |
...
| maestro | RED | last cycle 4409 min ago (`INTENT-20260904-212911-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 250h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 284h old (bar 30h) |
| delivery | RED | 651 commits on no remote (oldest 15.3d), 37 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 352 commits no remote holds, oldest 15.3d, 0 dirty |
| &nbsp;&nbsp;stranded | idp | 263 commits no remote holds, oldest 4.5d, 2 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 14.1d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main e85c83d4, 5 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2008 producers, 8823 measurables, 370 in gaps with a ticket, 18 unexplained, blind: cluster_live (`science/datamap.py --check`) |
| &nbsp;&nbsp;violation | | source ledger: owner ~/.claude/scripts/goal-guard.py does not exist |
| &nbsp;&nbsp;violation | | 18 producer(s) UNEXPLAINED (first: mac/ledger/~/.estate/claims.jsonl) |
| &nbsp;&nbsp;violation | | domain cluster_live BLIND and not allowed: RuntimeError: no receipt body in the last 8 oke-check.yml run(s): j |
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 341h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 10368 runs in 24h, 56 refused (most: rule-guard.py 38), slowest 20435 ms, 4 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 221.8h ago (bar 194h)) |
...
| OKE flux | RED | 10 of 78 Kustomizations not ready: chaos, commerce, gvisor-runtime, healthchecks, mcp, nodesoftware-operator, observability, science, temporal, To |
| elite grade | 19 GAP, 13 BLIND | 376 ELITE of 408 entities; page docs/SHOWCASE.md in idp, gaps first (crew#474) |
