...
**Generated 2026-08-31 09:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.60s |
...
| maestro | GREEN | last cycle 3 min ago (`INTENT-20260831-085357-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 68h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 102h old (bar 30h) |
| delivery | RED | 1578 commits on no remote (oldest 9.0d), 23 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 846 commits no remote holds, oldest 6.6d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 351 commits no remote holds, oldest 7.7d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 9.0d, 3 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 2 commit(s) to origin/main 55a3847, 3 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 159h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 51347 runs in 24h, 322 refused (most: pre-push 103), slowest 28177 ms, 29 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 39.8h ago (bar 194h)) |
