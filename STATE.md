...
**Generated 2026-09-03 11:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 17.66s |
...
| maestro | GREEN | last cycle 7 min ago (`INTENT-20260903-105211-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 142h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 176h old (bar 30h) |
| delivery | RED | 368 commits on no remote (oldest 10.8d), 23 dirty files, 6 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 352 commits no remote holds, oldest 10.8d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 9.6d, 0 dirty |
| &nbsp;&nbsp;stranded | estate-secrets | 5 commits no remote holds, oldest 8.7d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 2 commit(s) to origin/main 1cc63dc, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 4077 producers, 33908 measurables, 283 in gaps with a ticket, 18 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 233h ago (`science/warehouse.db`) |
...
| research | GREEN | 1 entry in 7d, 31 total, 31 with a decision fed, last 2026-08-28 (`RESEARCH-LEDGER.jsonl`) |
| hooks | GREEN | 43105 runs in 24h, 286 refused (most: rule-guard.py 154), slowest 61222 ms, 65 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 113.8h ago (bar 194h)) |
