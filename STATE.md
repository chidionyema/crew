...
**Generated 2026-09-09 23:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.93s |
...
| maestro | RED | last cycle 590 min ago (`INTENT-20260909-131344-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 298h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 332h old (bar 30h) |
| delivery | RED | 773 commits on no remote (oldest 17.3d), 42 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 373 commits no remote holds, oldest 6.5d, 34 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 17.3d, 0 dirty |
...
| live checkout | GREEN | moved 1 commit(s) to origin/main 57af3b6, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2208 producers, 9788 measurables, 352 in gaps with a ticket, 20 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 389h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 14881 runs in 24h, 105 refused (most: rule-guard.py 55), slowest 43007 ms, 11 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 269.8h ago (bar 194h)) |
