...
**Generated 2026-09-03 07:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 1.87s |
...
| maestro | GREEN | last cycle 3 min ago (`INTENT-20260903-065424-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 138h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 172h old (bar 30h) |
| delivery | RED | 1836 commits on no remote (oldest 11.9d), 24 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 1122 commits no remote holds, oldest 9.5d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 352 commits no remote holds, oldest 10.7d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 288 commits no remote holds, oldest 11.9d, 4 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 5055f7c, 4 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 229h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 41505 runs in 24h, 281 refused (most: rule-guard.py 144), slowest 61222 ms, 50 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 109.8h ago (bar 194h)) |
