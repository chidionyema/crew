...
**Generated 2026-09-10 05:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 2.24s |
...
| estate board | GREEN | estate-board-sync: 236 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 110 min ago (`INTENT-20260910-031404-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 304h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 338h old (bar 30h) |
| delivery | RED | 773 commits on no remote (oldest 17.6d), 44 dirty files, 10 live repos (`git log --branches --not --remotes`) |
...
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 17.6d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 16.4d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main bce1669, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2211 producers, 9803 measurables, 354 in gaps with a ticket, 20 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 395h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 10981 runs in 24h, 56 refused (most: rule-guard.py 28), slowest 43007 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 275.8h ago (bar 194h)) |
...
| OKE flux | RED | 22 of 82 Kustomizations not ready: agent-workforce, backstage, chaos, commerce, commerce-data, cyrus, dagster, estate-db, estate-db-migrate, guacamole, healing-analyzer, healing-k8sgpt, healthchecks, hindsight, llm, mcp, observability, otto-gateway, research-engine, science, temporal, To |
