...
**Generated 2026-08-30 17:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | GREEN | last cycle 3 min ago (`INTENT-20260830-165414-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 52h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 86h old (bar 30h) |
| delivery | RED | 1523 commits on no remote (oldest 8.3d), 47 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 828 commits no remote holds, oldest 5.9d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-vocab | 351 commits no remote holds, oldest 7.1d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 8.3d, 3 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 58e7265, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 5221 producers, 46054 measurables, 269 in gaps with a ticket, 16 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 143h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 51642 runs in 24h, 421 refused (most: rule-guard.py 142), slowest 35866 ms, 21 overturned by a marker (`hook-outcomes.jsonl`) |
| GitHub Actions | GREEN operational | githubstatus.com; a red row means pending CI is theirs, not yours |
| OCI verification identity | RED 1/1 scheduled runs failed | idp verify-drill.yml scheduled runs, last 24h, on the estate-ci machine identity (crew#345) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 23.8h ago (bar 194h)) |
