...
**Generated 2026-09-11 23:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260911-211212-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 346h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 380h old (bar 30h) |
| delivery | RED | 784 commits on no remote (oldest 19.3d), 10 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 383 commits no remote holds, oldest 8.5d, 3 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 19.3d, 0 dirty |
...
| live checkout | GREEN | moved 1 commit(s) to origin/main 6c8a2ee, 3 local edit(s) kept |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 437h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 599 runs in 24h, 5 refused (most: pre-commit 4), slowest 18551 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 317.8h ago (bar 194h)) |
...
| OKE flux | RED | 8 of 84 Kustomizations not ready: chaos, commerce, cyrus, guacamole, notify, tailscale, temporal, To |
