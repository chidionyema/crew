...
**Generated 2026-09-08 05:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 2.02s |
...
| maestro | STALE | last cycle 111 min ago (`INTENT-20260908-031218-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 256h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 290h old (bar 30h) |
| delivery | RED | 658 commits on no remote (oldest 15.6d), 44 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 357 commits no remote holds, oldest 15.6d, 0 dirty |
| &nbsp;&nbsp;stranded | idp | 265 commits no remote holds, oldest 4.8d, 10 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 14.4d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 29b043cf, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2024 producers, 8907 measurables, 372 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 347h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 21536 runs in 24h, 104 refused (most: rule-guard.py 73), slowest 25508 ms, 6 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 227.8h ago (bar 194h)) |
...
| OKE flux | RED | 15 of 78 Kustomizations not ready: backstage, chaos, commerce, gvisor-runtime, healthchecks, monitoring, nodesoftware-operator, observability, otto-gateway, otto-golden, reloader, science, tailscale, temporal, To |
