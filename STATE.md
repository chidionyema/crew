...
**Generated 2026-09-11 05:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 6.13s |
...
| maestro | RED | last cycle 592 min ago (`INTENT-20260910-191218-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 328h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 362h old (bar 30h) |
| delivery | RED | 776 commits on no remote (oldest 18.6d), 15 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 376 commits no remote holds, oldest 7.8d, 7 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 18.6d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 17.4d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 6066981, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2352 producers, 10407 measurables, 372 in gaps with a ticket, 35 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 419h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 16598 runs in 24h, 39 refused (most: rule-guard.py 21), slowest 22020 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 299.8h ago (bar 194h)) |
...
| OKE flux | RED | 30 of 84 Kustomizations not ready: agent-workforce, alerts, alerts-secret, backstage, chaos, commerce, commerce-data, crossplane-providerconfig, crossplane-providers, cyrus, dagster, estate-db, estate-db-migrate, guacamole, gvisor-runtime, healing-analyzer, healing-k8sgpt, healthchecks, hindsight, human-vault-bridge, identity, llm, nodesoftware-operator, observability, research-engine, science, tailscale, temporal, weave-gitops, To |
