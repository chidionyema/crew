...
**Generated 2026-08-28 21:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | GREEN | last cycle 2 min ago (`INTENT-20260828-205632-0d20f3e7.json`) |
...
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 42h old (bar 30h) |
| delivery | RED | 1448 commits on no remote (oldest 6.5d), 54 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 679 commits no remote holds, oldest 4.1d, 1 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 371 commits no remote holds, oldest 6.5d, 5 dirty |
| &nbsp;&nbsp;stranded | scripts | 349 commits no remote holds, oldest 5.2d, 21 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 2 commit(s) to origin/main 66f8012, 5 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 6952 producers, 62843 measurables, 221 in gaps with a ticket, 16 unexplained, blind: none (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 99h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 127408 runs in 24h, 682 refused (most: idle-guard.py 172), slowest 104612 ms, 30 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 2/38 layers on a cluster with no OCI (floor 2) (idp run 33208911991, 0.5h ago (bar 194h)) |
| &nbsp;&nbsp;second provider | | provider=github-hosted-azure distro=k3s wall_clock=672s cost=£0.00 (public repository, GitHub-hosted ubuntu-latest) |
