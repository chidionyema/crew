...
**Generated 2026-09-10 17:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 5.82s |
...
| maestro | STALE | last cycle 108 min ago (`INTENT-20260910-151356-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 316h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 350h old (bar 30h) |
| delivery | RED | 777 commits on no remote (oldest 18.1d), 30 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | NOT RUN | `gh search prs --merged` did not answer |
| &nbsp;&nbsp;stranded | idp | 377 commits no remote holds, oldest 7.3d, 23 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 18.1d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 16.9d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main b4d4bf3, 3 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 2257 producers, 10040 measurables, 357 in gaps with a ticket, 20 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 407h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 18487 runs in 24h, 54 refused (most: rule-guard.py 30), slowest 22596 ms, 0 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 287.8h ago (bar 194h)) |
...
| OKE flux | RED | 21 of 84 Kustomizations not ready: agent-workforce, backstage, chaos, commerce, commerce-data, cyrus, dagster, estate-db, estate-db-migrate, guacamole, healing-analyzer, healing-k8sgpt, healthchecks, hindsight, llm, observability, research-engine, science, tailscale, temporal, To |
