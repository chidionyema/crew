...
**Generated 2026-09-09 11:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 4.60s |
...
| estate board | GREEN | estate-board-sync: 225 row(s) from chidionyema/crew#102 -> /Users/chidionyema/.claude/ESTATE_BOARD.jsonl |
| maestro | STALE | last cycle 111 min ago (`INTENT-20260909-091246-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 286h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 320h old (bar 30h) |
| delivery | RED | 773 commits on no remote (oldest 16.8d), 27 dirty files, 10 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 373 commits no remote holds, oldest 6.0d, 19 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 364 commits no remote holds, oldest 16.8d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 15.6d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 8790421, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1957 producers, 8538 measurables, 380 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 377h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 13489 runs in 24h, 117 refused (most: rule-guard.py 65), slowest 25530 ms, 34 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 257.8h ago (bar 194h)) |
...
| OKE flux | RED | 63 of 83 Kustomizations not ready: agent-workforce, alerts, alerts-github, alerts-secret, autoscaler, backstage, chaos, chaos-mesh, cluster-state, commerce, commerce-data, crossplane, crossplane-providerconfig, crossplane-providers, cyrus, dagster, dns, drills, estate-db, estate-db-migrate, event-bus, external-secrets, flux-webhook, guacamole, gvisor-runtime, healing, healing-analyzer, healing-k8sgpt, healthchecks, hermes-agent, hindsight, human-vault, human-vault-bridge, identity, image-automation, keda, llm, mcp, metrics-server, monitoring, monitoring-rules, nodesoftware-operator, notify, observability, observability-collector, otto-gateway, otto-golden, otto-golden-secret, prospector, prospector-platform, reloader, research-engine, robusta, sandbox-launch, scheduling, science, secret-store, spire, tailscale, temporal, verification, weave-gitops, To |
