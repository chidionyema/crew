...
**Generated 2026-09-09 03:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
|---|---|---|
| The Architect | GREEN | `bin/verify`: PASS  dispatch claims agent-go and never icebox 3 passed in 3.25s |
...
| maestro | RED | last cycle 231 min ago (`INTENT-20260908-231208-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 278h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 312h old (bar 30h) |
| delivery | RED | 764 commits on no remote (oldest 16.5d), 15 dirty files, 9 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | idp | 371 commits no remote holds, oldest 5.7d, 8 dirty |
| &nbsp;&nbsp;stranded | .wt-cg-656 | 357 commits no remote holds, oldest 16.5d, 0 dirty |
| &nbsp;&nbsp;stranded | .wt-estate-656 | 11 commits no remote holds, oldest 15.3d, 0 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | GREEN | moved 1 commit(s) to origin/main 909c463, 4 local edit(s) kept |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 1896 producers, 8251 measurables, 376 in gaps with a ticket, 19 unexplained, blind: cluster_live (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 369h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 25779 runs in 24h, 257 refused (most: pre-commit 120), slowest 23571 ms, 35 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | RED | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 249.8h ago (bar 194h)) |
...
| OKE flux | RED | 8 of 82 Kustomizations not ready: chaos, commerce, crossplane-providerconfig, gvisor-runtime, healing-analyzer, science, temporal, To |
