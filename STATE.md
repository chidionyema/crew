...
**Generated 2026-08-30 09:03 UTC** by `scripts/estate-snapshot`. Every row is a command and its
...
| maestro | GREEN | last cycle 3 min ago (`INTENT-20260830-085344-0d20f3e7.json`) |
...
| revenue | NOT RUN | last measurement 2026-08-28T12:37:36Z is 44h old (bar 24h) |
| ci runs | NOT RUN | last measurement 2026-08-27T03:28:24Z is 78h old (bar 30h) |
| delivery | RED | 1502 commits on no remote (oldest 8.0d), 49 dirty files, 7 live repos (`git log --branches --not --remotes`) |
| &nbsp;&nbsp;shipped | >=1000 merged | non-bot PRs merged across the estate in 7d (`gh search prs`) |
| &nbsp;&nbsp;stranded | .idp-state | 808 commits no remote holds, oldest 5.6d, 1 dirty |
| &nbsp;&nbsp;stranded | .wt-vocab | 351 commits no remote holds, oldest 6.7d, 0 dirty |
| &nbsp;&nbsp;stranded | .crew-state | 287 commits no remote holds, oldest 8.0d, 5 dirty |
| founder cost | NOT RUN | `attention_daily` did not answer |
| live checkout | RED | 7 commit(s) behind origin/main; not moved: 1 edited file(s) the update also changes (docs/science/RESEARCH-GRADE.md) |
| collectors | NOT RUN | `ingest_log` did not answer |
| data map | RED | 5388 producers, 46532 measurables, 262 in gaps with a ticket, 16 unexplained, blind: none (`science/datamap.py --check`) |
...
| science plane: warehouse | RED | DuckDB+dbt, 0 dbt model(s), rebuilt 135h ago (`science/warehouse.db`) |
...
| hooks | GREEN | 85003 runs in 24h, 863 refused (most: rule-guard.py 191), slowest 40422 ms, 18 overturned by a marker (`hook-outcomes.jsonl`) |
...
| portability | GREEN | ready 11/43 (root-red 2 all named, cascaded 27, pending 3) on a cluster with no OCI (floor 10); the cascade waits 22 on secret-store, 3 on observability, 2 on prospector-platform (idp run 33265235988, 15.8h ago (bar 194h)) |
