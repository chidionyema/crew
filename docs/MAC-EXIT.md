...
| Portal + catalog | Backstage | `idp/clusters/oke/platform.yaml` Kustomization `backstage` |
...
| Front door | Gateway API + external-dns (`edge.yaml`) | `idp/clusters/oke/edge.yaml` |
...
| Agent runtime image | buildable locally (`.venv`, `hermes-v2/hermes-agent/docker-compose.yml`) | `Dockerfile` → `hermes-v2/.github/workflows/build-agent-image.yml` → `ghcr.io/chidionyema/hermes-agent` (arm64 for the Ampere pool); `gateway.yaml:90` pulls `set-by-an-overlay` | **Cloud home exists**; the one component with a real deploy path |
...
| Old label `ai.hermes.gateway` | three `.bak` plists beside the live `ai.architect.gateway.plist` — archived to `~/.claude/state/archive/launchagents-dead-2026-08-27.tgz` this turn (CP7) | — | guarded by `hermes-v2/tests/test_incident_crew284_one_gateway_label.py` |
