...
| 2.4 | **The day-after-the-sale drill** | idea 20; the 9D sentence | `idp/drills/catalogue.yaml` entry `founder-unreachable-72h`: the gateway blackholes founder-bound messages for 72 h in a rehearsal; the row grades receipts landed, asks raised (must be 0), incidents repeated (0), digest produced on return | the last untested assumption: that it runs without him | drill row GREEN in `bin/idp-verify` · founder receipt: one tap on the return digest | idp |
