...
| Event bus | NATS JetStream via the official nats-io chart (Apache-2.0) behind a transactional outbox; CloudEvents binary + protobuf; operator-mode accounts | designed (crew#639, ADR idp 0012; chart in `platform/event-bus`, dark on idp#800) | Founder specification 2026-08-29: durability lives in the contracts (subject grammar, envelope, outbox, idempotency), so the broker is replaceable; Kafka and CDC are deferred with named triggers |
| Platform daemons | Go 1.23 modules under `idp/platform/<name>`, multi-arch images on ghcr.io | designed (crew#639) | Founder 2026-08-29: "Golang is preferred stack for this day 0 layer"; one static binary passes Kyverno non-root read-only admission without a runtime |
