...
| Secrets | sops+age directory vault (founder ruling 2026-08-24, crew#119 comment 5393502867): one file per secret, env-segregated `secrets/<env>/<name>.yaml`, ingress `secret-add` reading the value from stdin — never argv — egress `secret-load`, k8s path via secrets-as-files with rotation-safe precedence (file wins); the vault lives in a PRIVATE repo — crew, prospector and hermes-v2 are public and no ciphertext lands in them (home: github.com/chidionyema/estate-secrets, executed 2026-08-24) | ruled; live 2026-08-24 — vault in the PRIVATE repo `estate-secrets` (main 598c36b, 24 keys); consumer cutover and blob retirement pending | Ruled the same day this row was measured adrift. The previous cell said "sops + age … live" while nothing sops-shaped existed (no `.sops.yaml` anywhere under ~/dev/code; a probe for it finds an age store never, which misled three sessions in one day) — "live" was the drift; the shape survived by founder ruling. Live today is the migration SOURCE: `prospector-main/deploy/secrets.env.age` (age X25519 whole-file, disk-local and untracked on purpose — prospector commit b5060a15 — identity `~/.config/prospector/age-key.txt` mode 600, 25 keys incl. MINIMAX_API_KEY); it stays live until the vault holds all 25, then is deleted per the ruling. Executed 2026-08-24 (session chidionyema-38; receipts crew#119 comment 5393860612): 24 keys in `estate-secrets:secrets/dev/<KEY>.yaml`, ingress `scripts/secret-add` (stdin only), egress `scripts/secret-load`, pre-commit plaintext refusal and pre-push visibility refusal proved both ways; crew's interim untracked copies removed. MINIMAX_API_KEY excluded (bogus value, real key awaited). The age blob stays live for existing consumers until cutover lands; the vault is the source of truth for new work. k8s note: ESO has no sops provider — the bridge is Flux/kustomize native sops. A fresh clone needs `git config core.hooksPath hooks`. Carried-over handling rulings: no private key is ever committed; any CI secret-scan ships report-mode first scoped past prospector's 152 legitimate matches (LAW 38). prospector#701 (recipients file, S5) is transitional — it protects the migration source and merges only if migration is not immediate; escrow (S1) remains founder action. Today's risk is LOSS, not exposure; residual: anything ever pushed to a public repo may stay fetchable by SHA, unenumerable from here |
