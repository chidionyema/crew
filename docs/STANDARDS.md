...
| Front door | One hostname per service under the estate DNS zone, published by external-dns from the Gateway API HTTPRoute (ADR 0001) with OIDC at the gateway (ADR 0003); the portal (`catalogue.<zone>`) is the only place a URL lives, every catalogue entity carries `links:`; a gate refuses an entity without a URL and a Service without discovery labels | proposed (crew#269) | Founder 2026-08-25: he must never ask for a URL and a migration must not change a name. `.local` rejected (mDNS, Mac-bound); `/etc/hosts` rejected (R35, one laptop); a register-on-boot script rejected (headline rule 1: GitOps + external-dns already do it). Zone: founder decision, default the zone external-dns already automates |
