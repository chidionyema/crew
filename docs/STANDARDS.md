...
| GitOps | Flux v2 on OKE: `idp/clusters/oke/` is the tree Flux reconciles, `idp/bin/idp-flux-bootstrap` installs it | live (measured 2026-08-26: oke-check apply run 32993341333 PASS; the catalogue image rolls on every main build through Flux image automation, idp#222) | Recommended for solo-operator clusters; CNCF Graduated, Apache-2.0. Argo CD is the reviewed deviation (97% of surveyed users run it in production — [CNCF 2025 Argo CD End User Survey](https://www.cncf.io/announcements/2025/07/24/cncf-end-user-survey-finds-argo-cd-as-majority-adopted-gitops-solution-for-kubernetes/)) but it is a UI service to run. Row corrected by crew#325: it said "not yet" while Flux was live |
