...
| Secrets | age (whole-file) via `prospector-main/deploy/secrets.sh`; store is disk-local (`deploy/secrets.env.age`, untracked on purpose — commit b5060a15), identity `~/.config/prospector/age-key.txt`; in-cluster later: secrets-as-files, rotation-safe precedence (file wins) | live | Corrected 2026-08-24: this row said "sops + age" while the live implementation is age alone — secrets.sh's header records that decision (whole-file encryption, no field-level need; a `.sops.yaml` probe finds nothing by construction, which misled three sessions in one day). sops is the stated deviation path only if field-level YAML encryption is ever needed. Measured: 25 keys via `secrets.sh list`, MINIMAX_API_KEY among them. Open gaps in dependency order (SECRETS_PROGRAM.md 3.7): no `deploy/secrets.recipients` file — `encrypt_stdin` re-encrypts to a single recipient (S5; fix in review as prospector#701) — then no escrow (S1, founder action). Today's risk is LOSS, not exposure; residual: anything ever pushed to the public repo may stay fetchable by SHA, unenumerable from here. pydantic-settings' env-over-file precedence remains the wrong direction for rotation |
