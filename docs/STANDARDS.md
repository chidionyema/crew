...
| Scheduling | Dagster (Apache-2.0, self-hosted) on the cluster via the dagster/dagster Helm chart, one code location `estate_scheduler`; GitHub Actions `schedule:` only for CI-shaped clocks | partially live | Founder ruling R67 (2026-09-01): the Mac scheduler moves into the cluster first (crew#716). Argo Workflows is the rejected alternative: a second scheduler beside the one that already holds every job is the stitching the headline forbids, and every clock in the estate is listed on one generated page, `idp/docs/scheduling/CLOCKS.md` (bin/estate-clocks) |
