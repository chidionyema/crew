...
| GitOps | Flux v2 on OKE: `idp/clusters/oke/` is the tree Flux reconciles, `idp/bin/idp-flux-bootstrap` installs it | live (measured 2026-08-26: the `check` job of oke-check run 32993341333 applied and graded PASS; that run's `chaos-drill` job is red until its first Schedule fire on 2026-08-31; the catalogue image rolls on every main build through Flux image automation, idp#222) | Recommended for solo-operator clusters; CNCF Graduated, Apache-2.0. Argo CD is the reviewed deviation (97% of surveyed users run it in production — [CNCF 2025 Argo CD End User Survey](https://www.cncf.io/announcements/2025/07/24/cncf-end-user-survey-finds-argo-cd-as-majority-adopted-gitops-solution-for-kubernetes/)) but it is a UI service to run. Row corrected by crew#325: it said "not yet" while Flux was live |
...
| Board sync (read) | `bin/estate-board-sync` (a 3-line exec shim) over `scripts/estate-board-sync.py` | live | `gh issue view --json comments` is what the script calls, then a tmp-then-rename atomic write rebuilds `~/.claude/ESTATE_BOARD.jsonl` as the offline cache. `scripts/estate-snapshot` invokes it as the `board_sync()` row on every snapshot, so the same entry point humans use ad-hoc is the one the scheduled job uses. The board of record is chidionyema/crew#102; the JSONL is the second store, never the first. NOT a wheel to reinvent: no mature OSS issue cache survives (a) offline (laptop asleep, no `gh`) and (b) GitHub anonymous rate limits, and the cache exists so reads don't have to hit the API. Failure to land a row on the issue is dead-lettered to `~/.claude/state/board-deadletter.jsonl` and warned loudly — never dropped silently. The wrapper is `exec` so the script stays one place. |
| Secrets | On the cluster: External Secrets Operator over OCI Vault (`idp/platform/secret-store`, `idp/platform/secrets`, ingress `idp/bin/idp-vault-put`); every Kubernetes Secret is an ExternalSecret, no ciphertext in git. Git-side and laptop-side: the sops+age directory vault ruling of 2026-08-24 (crew#119 comment 5393502867) stands for the private `estate-secrets` repo and the prospector migration source | live on OKE (ESO + OCI Vault, measured 2026-08-26: `idp/clusters/oke/secrets.yaml` reconciles); sops vault migrating | Row corrected by crew#325: it described only the sops ruling while OCI Vault was what ran. Rules carried over: no private key is ever committed; a CI secret scan ships report-mode first (LAW 38); a value enters through stdin, never argv. Residual: anything ever pushed to a public repo may stay fetchable by SHA |
