...
| finding |
|---|
| **No Windmill config exists anywhere in the repo** (`grep -ril windmill` across `idp/` returns nothing outside old worktree prose docs). Team memory records a founder ruling that Windmill is the named fallback for Temporal in infra (crew#695), but the config on disk still runs **Temporal** (`platform/temporal/temporal.yaml`, `worker.yaml`) and schedules through **Dagster** (`scheduler/dagster.yaml`, `schedule.yml`), not Windmill. This is a decision-vs-config gap worth surfacing plainly: the ruling was made, nothing was built or migrated. |
...
`idp/platform/secret-store/store.yaml`, `secrets/external-secrets.yaml`
