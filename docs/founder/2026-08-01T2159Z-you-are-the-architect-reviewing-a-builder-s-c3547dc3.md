---
captured: 2026-08-01T21:59:24+00:00
session: d79e8258-5f9d-48c4-844e-8c02545d2485
...
The spec is at `specs/ux-heuristic-2026-08-01.md`. **Read it in full first — every numbered item, every file, every acceptance clause is the contract.** Do not paraphrase it.

The failing test is already written at `store_platform/src/Store.Web/src/__tests__/uxHeuristicContract.test.ts`. Read it too — it encodes exactly what "done" means, item by item. **Do not modify the test file.** It is protected; restoring byte-for-byte at verify time.

### Branch & state

Working branch is `ux-heuristic-2026-08-01` (already created from `main`). Runtime artifacts
(`store/scheduler/audit/*.jsonl`, `store/provider_health*.json`, `store/control_center/config_history.jsonl`,
`store/scheduler/DIAGNOSTICS_LATEST.txt`, `storage/durable_ledger.md`) are dirty — **DO NOT
`git add` or commit any of those.** Only `git add` the files you intend to change.

### Procedure — four commits, in this order

1. **`store: account link gets visual weight in the header`** — `components/marketing/MarketingLayout.tsx` only. The `/account` `<Link>` (around lines 85-89) gains a hairline border + small rounded container: `inline-flex items-center gap-1.5 ... rounded-md border border-border px-3 py-1.5 text-sm font-semibold text-text hover:bg-bg`. Icon and label stay. Nothing else in the header changes. Commit.

2. **`store: move email capture from before the catalogue to after`** — `pages/index.tsx`. Cut the `<WaitlistForm source="home-after-sample" … />` block (currently between the sample section and the catalog, lines ~909-924) and paste it AFTER the catalog grid closes but BEFORE the final `<CtaBand>`. The `source` tag stays `"home-after-sample"`. Commit.

3. **`store: FAQ — three categories, plus the format and competition questions`** — two files.
   - `lib/faqContent.ts` — add a `category: 'packs' | 'payment' | 'process'` field to every existing item (10 of them). Add two new items:
     - category `'packs'`, question about delivery format, answer that names the zip of plain Markdown files, links to `/sample` and to `/pack/<id>#table-of-contents` for the per-pack format. **Do NOT invent a screenshot — link to the existing pack-page preview.**
     - category `'packs'`, question about 500 other buyers copying the same idea, answer that names the *honest* moat (niche sizing, route specificity, buyer profile granularity) and **cites `/kill-log` in plain text** so the reader can see how many candidates die on those gates. **Do NOT fabricate scarcity numbers** ("only 100 copies" or similar).
   - `pages/faq.tsx` — split the single "Buying a pack" section into three sections using the new category field. Each section keeps the same card styling (`bg-white border border-border p-8 rounded-lg …`) and keeps the sticky contact panel. The `FAQPage` JSON-LD must continue to enumerate ALL 12 items via `FAQS.map(...)` — DO NOT filter the schema. Commit.

4. **`store: how-it-works — stepped timeline, the six checks, and the graveyard`** — `pages/how-it-works.tsx`. Replace the four wall-of-text sections (current lines ~36-105) with:
   - **A stepped timeline** of the six gates (one card each, in this order): `pain_reality`, `value_durability`, `incumbency`, `payer_solvency`, `route_to_market`, `legality`. Each card carries:
     - The gate name as a heading.
     - The full `gateLabel` from the JSON as the subtitle.
     - **One inline example** pulled from `data/kill-log.json` — the six titles named in the spec: NI-GapSweep, DecibelKit, SaltCourt, SplitCare, AssessAid, GasSafe. Show the entry's `reason` truncated to ~160 chars and a `See kill-log` link.
     - Each card links to `/kill-log` for the full entry.
   - **A short adversarial-pass paragraph** (not a wall of text — 2-3 sentences max).
   - **The graveyard section** — one sentence: "Of 960 ideas researched, 103 survived" plus a CTA block linking to `/kill-log`. **Do NOT inline the full kill-log card markup** — link out. The contract test guards against inlining extra titles.
   - **The honest-limits paragraph** preserved verbatim from the current file.
...
End with `echo "UX_HEURISTIC_OK"` on success.

### Failure modes to watch for (from previous attempts)

- **JSON-LD schema preservation** — the FAQ's `FAQPage` rich-result eligibility depends on the schema matching the visible text. If you accidentally filter the schema to only one category, Google drops the rich result. Verify by reading `pages/faq.tsx` line by line after editing.
...
- **`/data/kill-log.json`** is a static import. Pull only the six example entries specified in the spec; do NOT inline the full array.
- **Existing FAQPage schema in `_document.tsx`** — leave it alone. It emits a brand-level `Organization + WebSite` graph, unrelated to FAQ.
- **Existing Copy in the honest-limits section** — preserve verbatim. The contract test guards the literal phrase.
- **`source="home-after-sample"`** — keep the tag, even though the position changed. The `source` value is for analytics attribution, not for placement.

### When you finish

Print:
- The four commit SHAs with one-line summaries.
- The full file list you changed, with line counts added/removed per file.
- The verify-chain output, last ~30 lines.
- A one-line confirmation that `echo "UX_HEURISTIC_OK"` printed.
...
index f4229a6..7b03d78 100644
--- a/store/control_center/config_history.jsonl
+++ b/store/control_center/config_history.jsonl
@@ -550,3 +550,15 @@ backup: /private/var/folders/gq/jbt0858s1g99w05n7k7531040000gn/T/pytest-of-chidi
 hash: 78814b94251c
