...
the 191 rows that existed before the board became this issue"*. They are prose, they were
never broadcasts, and the sync leaves them out.
...
The path is canonical: parent directory `~/.claude/state/`, file name
`board-deadletter.jsonl`, absolute. The path itself is part of the contract — a session
cannot rename the file and lose the failure channel by accident. Each entry carries at
minimum:
