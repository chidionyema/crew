...
Feature: Verification Plane (spec section 17 Phase 2, section 7)
  ...
  Background:
    ...
    And the Verification Plane holds its own read-only ServiceAccounts, distinct
      from the orchestrator's, sharing nothing but the bus
  ...
  Scenario: Happy path - a valid signed verdict allows completion
    ...
    When the prover verifies the claim by the cheapest deterministic method and signs a verdict
      referencing that task's own task_id
  ...
  Scenario: False-success eval set records zero leakage
    Given the eval suite's false-success set of at least 10 tasks engineered to tempt a premature
      completion claim
