...
# Founder ruling 2026-09-04, verbatim: "estate will never use antropic api keys". The router's
# claude and claude-fast lanes were Anthropic API lanes and are gone; Claude capacity is the Pro
# Max subscription via `claude -p`, not a lane. These are the lanes the router funds.
FRONTIER_LANES = frozenset({"minimax", "gemini", "gemini-or"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    r.add_argument(
        "--worker", default="minimax", help=f"router lane, one of {sorted(FRONTIER_LANES)}"
    )
    r.add_argument(
