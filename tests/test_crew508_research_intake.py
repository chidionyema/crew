...
def test_first_pull_files_baselines_not_candidates():
    new, state = ri.pull(
        SRC, [], NOW, fake_fetch({"fluxcd/flux2": "v2.9.4", "duckdb/duckdb": "v1.5.5"})
    )
    assert [r["status"] for r in new] == ["baseline", "baseline"]
    assert (
        state["new"] == 2
        and state["unreachable"] == []
        and state["last_pull"] == "2026-08-27T15:00:00+00:00"
    )
...
def test_later_release_is_a_candidate_and_seen_release_is_not_refiled():
    ...
    new, state = ri.pull(
        SRC, rows, NOW, fake_fetch({"fluxcd/flux2": "v2.10.0", "duckdb/duckdb": "v1.5.5"})
    )
    assert [(r["repo"], r["tag"], r["status"]) for r in new] == [
        ("fluxcd/flux2", "v2.10.0", "candidate")
    ]
    assert state["new"] == 1
...
def test_unreachable_repo_is_named_and_does_not_stop_the_pull():
    ...
    assert state["unreachable"] == ["fluxcd/flux2"] and [r["repo"] for r in new] == [
        "duckdb/duckdb"
    ]
...
def test_candidate_unanswered_past_seven_days_is_late_and_answered_one_is_not():
    ...
    assert (
        [r["repo"] for r in g["late"]] == ["fluxcd/flux2"]
        and g["adopted"] == 1
        and g["candidates"] == 1
    )
...
def test_outward_grade_drops_to_gap_when_the_intake_is_stale_or_late():
    ...
    assert rg.grades(healthy, inw, {"fresh": True, "late": []}, delivered_n=1)[0] == "ELITE"
...
def test_check_exits_one_on_a_stale_pull(tmp_path, monkeypatch, capsys):
    ...
    (tmp_path / "st.json").write_text(
        json.dumps({"last_pull": dt.datetime.now(dt.UTC).isoformat()})
    )
...
def test_every_watched_row_names_a_standards_row():
    rows = {
        ln.split("|")[1].strip()
        for ln in (CREW / "docs" / "STANDARDS.md").read_text().splitlines()
        if ln.startswith("| ")
    }
