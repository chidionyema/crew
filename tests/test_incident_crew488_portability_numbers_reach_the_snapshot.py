...
K3S = (
    "ok      portability-k3s  provider=github-hosted-azure distro=k3s wall_clock=670s "
...
def _runs(*, created="2026-08-28T11:05:06Z", conclusion="success", status="completed"):
    return json.dumps(
        [
            {
                "databaseId": 33166470567,
                "conclusion": "",
                "createdAt": "2026-08-28T11:15:33Z",
                "status": "in_progress",
            },
            {
                "databaseId": 33165807800,
...
def test_a_green_run_with_no_grade_line_is_not_run_never_green():
    ...
    assert rows == [
        "| portability | NOT RUN | idp run 33165807800, 0.9h ago (bar 194h) is green "
        "but its log holds no `ok portability` grade line |"
    ], rows
...
def test_an_in_progress_run_is_never_the_one_that_is_graded():
    ...
    assert rows == [
        "| portability | NOT RUN | no successful run of `portability-drill.yml` "
        "(crew#488 CP1/CP2) |"
    ], rows
