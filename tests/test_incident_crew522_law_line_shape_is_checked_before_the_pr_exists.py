...
def test_the_two_refused_lines_are_refused_here_too():
    ...
    body = GOOD.replace("- LAW 4 calibration: docs/x.md",
    ...
    body = GOOD.replace("- LAW 1 zero-gravity: n/a: no new config",
...
def test_a_proof_shaped_line_passes_and_a_missing_line_does_not():
    ...
    ok, why = pe.architecture_laws(GOOD.replace("- LAW 2 fractal: python3 -m pytest -q tests/test_x.py\n", ""))
...
def test_check_wires_the_verdict_in():
    ...
    assert '{why_dod}, {why_laws};' in src
